...
const ShareModal = ({ isOpen, onClose, board }) => {
    ...
    return (
        <div className="fixed inset-0 z-[200] overflow-y-auto bg-slate-900/40 backdrop-blur-sm" onClick={onClose}>
            {/* Centering Wrapper */}
            <div className="min-h-full flex items-center justify-center p-4">
                <div
                    className="bg-white w-full max-w-[580px] rounded-2xl shadow-2xl flex flex-col relative my-8 animate-in fade-in zoom-in-95 duration-200 border border-slate-100"
                    onClick={e => e.stopPropagation()}
                >
                {/* Header */}
                <div className="flex items-center justify-between px-5 sm:px-8 pt-6 sm:pt-8 pb-4">
                    <h2 className="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight">Share board</h2>
                ...
                <div className="px-5 sm:px-8 pb-6 sm:pb-10 flex flex-col gap-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        <button
                            ...
                            className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 py-2.5 rounded-xl text-sm transition-all active:scale-95 shadow-lg shadow-blue-100 w-full sm:w-auto"
                        >
                            Share
                        </button>
                    </div>
                    ...
                    {/* Tabs */}
                    <div className="flex gap-6 border-b border-slate-100 -mx-1 overflow-x-auto no-scrollbar">
                        <button onClick={() => setActiveTab('members')} className={`pb-3 text-sm font-bold transition-all relative whitespace-nowrap ${activeTab === 'members' ? 'text-blue-600' : 'text-slate-400'}`}>
